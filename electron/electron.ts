...
const clientId =
  process.env.SOUNDCLOUD_CLIENT_ID || "342b8a7af638944906dcdb46f9d56d98";
const redirectUri =
  process.env.REDIRECT_URL || "http://sc-redirect.herokuapp.com/callback.html";
 ...
 */
const authenticateUser = (afterOpen = undefined) => {
  let contents;
  ...
  contents.on("did-navigate", (_, url) => {
    ...
    accessToken = url.substring(
      url.indexOf(access_tokenStr) + 13,
      url.indexOf(expires_inStr)
    );
